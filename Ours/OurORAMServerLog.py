...
class ORAMServer:
    ...
    def oramServerInitialization(self):
        ...
        self.tcpSoc.sendMessage(cutils.padToSize(str(len(self.ES)),self.BSSize)+cutils.padToSize(str(len(self.EB)),self.BSSize)+cutils.padToSize(str(self.full[0]),self.FullFlagSize))
    ...
    def oramServerAccess(self):
        ...
        if len(self.EB)>1:
            for i in range(len(self.EB)-1, 0, -1):
                self.tcpSoc.sendMessage(cutils.padToSize(self.EB[i][0],self.AddrSize)+str(self.EB[i][1]))
        if len(self.ES)>1:
            for i in range(1,len(self.ES)):
                self.tcpSoc.sendMessage(cutils.padToSize(self.ES[i][0],self.AddrSize)+str(self.ES[i][1]))
        
        if self.full[0]==1:
            prgK0, convertK0, k_00 = cutils.strToDpfKeys(self.tcpSoc.receiveMessage())
            Bi0 = cutils.dpfEvalAll(prgK0,convertK0,len(self.EC[0]),0,k_00)
            val0 = cutils.readWithPos(Bi0,self.EC[0],self.posDict[0][0])
            
            prgK1, convertK1, k_01 = cutils.strToDpfKeys(self.tcpSoc.receiveMessage())
        ...
        if len(self.EB)>1:
            ...
            prgK, convertK, k_0 = cutils.strToDpfKeys(self.receiveM())
    ...
    def oramServerRebuildFL(self):
        ...
        if len(tmpEB)>1:
            for i in range(len(tmpEB)-1, 0, -1):
                self.tcpSoc.sendMessage(cutils.padToSize(tmpEB[i][0],self.AddrSize)+str(tmpEB[i][1])) # send (k,v)
                self.tcpSoc.sendMessage(cutils.bytesToStr(tmpTB[i])) # send tag
        if len(tmpES)>1:
            for i in range(1,len(tmpES)):
                self.tcpSoc.sendMessage(cutils.padToSize(tmpES[i][0],self.AddrSize)+str(tmpES[i][1])) # Only Server1
                self.tcpSoc.sendMessage(cutils.bytesToStr(tmpTS[i]))              
        for i in dict.keys(tmpDict[0]):
            self.tcpSoc.sendMessage(cutils.padToSize(tmpEC[0][i][0],self.AddrSize)+str(tmpEC[0][i][1])) # Only Server1
            self.tcpSoc.sendMessage(cutils.bytesToStr(tmpTC[0][i]))
        for i in dict.keys(tmpDict[1]):
            self.tcpSoc.sendMessage(cutils.padToSize(tmpEC[1][i][0],self.AddrSize)+str(tmpEC[1][i][1])) # Only Server1
    ...
    def oramServerRebuild(self, lev):
        ...
        if len(tmpEB)>1:
            for i in range(len(tmpEB)-1, 0, -1):
                self.tcpSoc.sendMessage(cutils.padToSize(tmpEB[i][0],self.AddrSize)+str(tmpEB[i][1])) # Only Server1
        ...
        for j in range(self.ell+1,lev):
            for i in dict.keys(self.posDict[j-self.ell][0]):
                self.tcpSoc.sendMessage(cutils.padToSize(self.ET[j-self.ell][0][i][0],self.AddrSize)+str(self.ET[j-self.ell][0][i][1])) # Only Server1
                self.tcpSoc.sendMessage(cutils.bytesToStr(self.TT[j-self.ell][0][i]))
            for i in dict.keys(self.posDict[j-self.ell][1]):
                self.tcpSoc.sendMessage(cutils.padToSize(self.ET[j-self.ell][1][i][0],self.AddrSize)+str(self.ET[j-self.ell][1][i][1])) # Only Server1
    ...
    def oramServerRebuildL(self):
        ...
        self.full[self.L-self.ell]=1
        self.ellAccessTimes = 0
        self.tcpSoc.sendMessage(str(self.full[0])+str(len(self.ES)))
    ...
    def oramServerAccessOnlyRead(self):
        ...
        Receive EllEC
        """
        #receiveDpfKeyList = self.receiveM()
        #sendMessList = ""
        """
        Send
        """
        if len(self.EB)>1:
            for i in range(len(self.EB)-1, 0, -1):
                self.tcpSoc.sendMessage(cutils.padToSize(self.EB[i][0],self.AddrSize)+str(self.EB[i][1]))
                #sendMessList = sendMessList+cutils.padToSize(self.EB[i][0],self.AddrSize)+str(self.EB[i][1])
        if len(self.ES)>1:
            for i in range(1,len(self.ES)):
                self.tcpSoc.sendMessage(cutils.padToSize(self.ES[i][0],self.AddrSize)+str(self.ES[i][1]))
                #sendMessList = sendMessList+cutils.padToSize(self.ES[i][0],self.AddrSize)+str(self.ES[i][1])

        if self.full[0]==1:
            prgK0, convertK0, k_00 = cutils.strToDpfKeys(self.receiveM())
            Bi0 = cutils.dpfEvalAll(prgK0,convertK0,len(self.EC[0]),0,k_00)
            val0 = cutils.readWithPos(Bi0,self.EC[0],self.posDict[0][0])
            
            prgK1, convertK1, k_01 = cutils.strToDpfKeys(self.receiveM())
            Bi1 = cutils.dpfEvalAll(prgK1,convertK1,len(self.EC[1]),0,k_01)
            val1 = cutils.readWithPos(Bi1,self.EC[1],self.posDict[0][1])
            #tempDpfKeyInd += tempDPFKeyLen
            self.tcpSoc.sendMessage(cutils.padToSize(str(val0[0]),self.AddrSize)+str(val0[1]))
            self.tcpSoc.sendMessage(cutils.padToSize(str(val1[0]),self.AddrSize)+str(val1[1]))
        
            #sendMessList = sendMessList+cutils.padToSize(val0[0],self.AddrSize)+str(val0[1])+cutils.padToSize(val1[0],self.AddrSize)+str(val1[1])
        
        for lev in range(self.ell+1,self.L+1):
            if self.full[lev-self.ell]==0:
                continue
            #tempDPFKeyLen = self.getDPFKeySize(len(self.ET[lev-self.ell][0]))
            prgK0, convertK0, k_00 = cutils.strToDpfKeys(self.receiveM())   
            Bi0 = cutils.dpfEvalAll(prgK0,convertK0,len(self.ET[lev-self.ell][0]),0,k_00)
            val0 = cutils.readWithPos(Bi0,self.ET[lev-self.ell][0],self.posDict[lev-self.ell][0]) 
            #tempDpfKeyInd += tempDPFKeyLen
        ...
        #self.tcpSoc.sendMessage(sendMessList)


if __name__=="__main__":
    soram = ORAMServer()
    soram.oramServerInitialization()
    access_times = soram.access_times#513#4*soram.N-1
    bb = time.time()
