...
class ORAMServer:
    ...
    def oramServerAccess(self):
        ...
        if self.full[0]==1:
            ...
            Write to EllTC
            """
            modV0 = cutils.strToBytes(self.tcpSoc.receiveMessage())
            prgK, convertK, k_0 = cutils.strToDpfKeys(self.tcpSoc.receiveMessage())
            Bi = cutils.dpfEvalAll(prgK,convertK,len(self.TC[0]),1,k_0)
            cutils.pirWriteWithEvalAndPos(Bi,modV0,self.TC[0],self.posDict[0][0])

            modV1 = cutils.strToBytes(self.tcpSoc.receiveMessage())
            prgK, convertK, k_0 = cutils.strToDpfKeys(self.tcpSoc.receiveMessage())
