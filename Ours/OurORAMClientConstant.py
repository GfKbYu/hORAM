...
class ORAMClient:
    ...
    def __init__(self, N, BlockSize, access_times) -> None: #, c, ellCuckoo, countQ, countS, maxLevel all+1 dORAM.OldEmpty
        ...
        self.L = math.ceil(math.log2(N)) # 14
        self.ell = min(self.L-1, math.ceil(2*math.log2(math.log2(N)))) # 8
        self.ecEllLength = 2**(self.ell+1)+math.ceil(math.log2(N))*(self.L-self.ell) # 596
        self.ctr = 0
        ...
        self.ellTableLength = (int)((1+self.cuckooEpsilon)*self.ecEllLength) # 601
        self.LTableLength = (int)((1+self.cuckooEpsilon)*(2**self.L)) # 16547
        self.byteOfCom = 1024
    ...
    def oramClientAccess(self, op, a, writeV):
        ...
        if self.lenEBAndTB>1:
            ...
            self.tcpSoc0.sendMessage(cutils.bytesToStr(writeModTag))  # bytes: 16
            self.tcpSoc0.sendMessage(cutils.dpfKeysToStr((prfK, convertK, k_0))) # bytes: 121
            self.tcpSoc1.sendMessage(cutils.bytesToStr(writeModTag))  # bytes: 16
            self.tcpSoc1.sendMessage(cutils.dpfKeysToStr((prfK, convertK, k_1))) # bytes: 121
        if self.lenES>1:
            tempEPos = 0
        ...
        if self.full[0]==1:
            ...
            self.tcpSoc0.sendMessage(cutils.dpfKeysToStr((T0prfK, T0convertK, T0k_0))) # 229
        ...
        for lev in range(self.ell+1,self.L+1):
            ...
            self.tcpSoc0.sendMessage(cutils.dpfKeysToStr((T1prfK, T1convertK, T1k_0)))
            self.tcpSoc1.sendMessage(cutils.dpfKeysToStr((T0prfK, T0convertK, T0k_1)))
            self.tcpSoc1.sendMessage(cutils.dpfKeysToStr((T1prfK, T1convertK, T1k_1))) # 319
        if self.full[0]==1:
            ellLevelCipher = self.generateLevelCipher(self.ell, self.getEpoch(self.ell))
            pos_0, pos_1 = self.hashPosition(ellLevelCipher, tag, self.ellTableLength)
            
            temp_share_data_0 = self.receiveM(self.tcpSoc0)#temp_d0.split( ) # 32
